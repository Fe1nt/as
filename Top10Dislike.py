...
if __name__ == "__main__":
    ...
    filtedInfo = Original.map(filtering)    #filter the useful things out 
    integrated = filtedInfo.map(counting)   #do some count insides the record
    records = integrated.groupByKey().map(lambda x : (x[0], list(x[1])))    #get the Keys togethere
    result = records.map(countTwo)  #get first two days out and do some counting
